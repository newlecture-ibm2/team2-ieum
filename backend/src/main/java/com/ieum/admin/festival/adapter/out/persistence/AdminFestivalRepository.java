...
public interface AdminFestivalRepository extends JpaRepository<AdminFestivalEntity, Long> {
    ...
    @Query("SELECT f FROM AdminFestivalEntity f WHERE f.isCustom = false " +
           ...
           "AND (:categories IS NULL OR f.category IN :categories OR f.categoryMid IN :categories OR f.categorySub IN :categories) " +
    ...
    java.util.List<AdminFestivalEntity> findByIsCustomFalse();
}
