...
│   │
│   └── 📁 inquiry/                            # 📩 문의하기 (로그인 사용자 전용)
│       ├── 📁 adapter/ (in/web/ → /api/inquiries/**, out/persistence/)
│       ├── 📁 application/
│       │   ├── 📁 port/
│       │   │   ├── 📁 in/
│       │   │   │   ├── CreateInquiryUseCase.java
│       │   │   │   └── GetMyInquiryListUseCase.java
│       │   │   └── 📁 out/
│       │   │       ├── LoadInquiryPort.java
│       │   │       └── SaveInquiryPort.java
│       │   ├── 📁 result/
│       │   │   ├── InquiryListResult.java
│       │   │   └── InquiryDetailResult.java
│       │   └── 📁 service/
│       │       └── InquiryService.java
│       └── 📁 domain/                         ← shared/domain/model/Inquiry 사용
│
...
│   │   └── 📁 domain/                         ← shared/domain/model/Notice 사용
│   │
│   ├── 📁 inquiry-mgmt/                       # 📩 문의 관리 (목록 조회 + 답변 작성)
│   │   ├── 📁 adapter/ (in/web/ → /api/admin/inquiries/**, out/persistence/)
│   │   ├── 📁 application/
│   │   │   ├── 📁 port/
│   │   │   │   ├── 📁 in/
│   │   │   │   │   ├── GetInquiryAdminListUseCase.java
│   │   │   │   │   ├── GetInquiryDetailUseCase.java
│   │   │   │   │   └── AnswerInquiryUseCase.java
│   │   │   │   └── 📁 out/
│   │   │   │       ├── LoadInquiryPort.java
│   │   │   │       └── SaveInquiryAnswerPort.java
│   │   │   ├── 📁 result/
│   │   │   │   └── InquiryAdminResult.java
│   │   │   └── 📁 service/
│   │   │       └── InquiryAdminService.java
│   │   └── 📁 domain/                         ← shared/domain/model/Inquiry, InquiryAnswer 사용
│   │
│   ├── 📁 statistics/                         # 📊 통계 대시보드
│   │   ├── 📁 adapter/
│   │   │   ├── 📁 in/web/
...
└── 📁 global/                                 # 🔷 공통 인프라 (user/admin 모두 공유)
    ...
    │   │   # /api/inquiries/** → authenticated (USER)
...
| 8 | 👤 **마이페이지** | 즐겨찾기, 내 게시글, 내 리뷰, 내 문의 목록/답변 확인, 프로필 수정 | |
| 9 | 📩 **문의하기** | 로그인 사용자 전용, 문의 작성 및 내 문의 조회 | 회원 전용 |
| 10 | ⚙️ **관리자 대시보드** | 축제 데이터 관리(공공 API 갱신), 사용자 신고관리, 문의 관리·답변, 공지/팝업 설정 | 관리자 전용 |
| 11 | 📊 **통계 대시보드** | 지역별 축제 분포 현황, 사용자 유입/활동 추이 시각화 | 관리자 전용, 필수 |
| 12 | 📢 **공지사항** | 공지 목록 및 상세 | |
...
메인 페이지 → 회원가입/로그인 → 전국 축제(지도 탐색) → 축제 상세 → 즐겨찾기 추가
                              ...
                              ├→ 공지사항 → 공지 상세
                               ├→ 문의하기 → 문의 작성
                               └→ 마이페이지 → 즐겨찾기 / 내 게시글 / 내 문의(답변 확인) / 프로필 수정

[관리자 Flow]
관리자 로그인 → 관리자 대시보드 → 축제 데이터 관리 (공공 축제 관리 / 자체 축제 관리)
                               ├→ 운영관리 (사용자 신고관리 / 문의 관리·답변 / 공지·팝업 설정)
...
| # | 엔티티명 | 설명 | 주요 속성 |
|---|---------|------|----------|
| 1 | **User** | 사용자 정보 | id, login_id, password, nickname, profile_image, role, security_question, security_answer, created_at |
| 2 | **Festival** | 축제 정보 | id, title, description, start_date, end_date, location, address, latitude, longitude, category, thumbnail_url, extra_images, avg_rating, review_count, favorite_count, status, is_custom, created_at |
| 3 | **Review** | 축제 후기/별점 | id, user_id(FK), festival_id(FK), rating, content, created_at |
| 4 | **Favorite** | 즐겨찾기 | id, user_id(FK), festival_id(FK), created_at |
| 5 | **Post** | 커뮤니티 게시글 | id, user_id(FK), board_type, title, content, image_url, view_count, created_at |
| 6 | **Comment** | 게시글 댓글 | id, user_id(FK), post_id(FK), user_name, content, created_at |
| 7 | **Notice** | 공지사항 | id, admin_id(FK), category, title, content, is_popup, is_pushed, is_active, created_at |
| 8 | **Report** | 신고 | id, reporter_id(FK), target_type, target_id, reason, status, action, created_at |
| 9 | **Inquiry** | 문의하기 | id, user_id(FK), title, content, status, created_at |
| 10 | **InquiryAnswer** | 문의 답변 | id, inquiry_id(FK), admin_id(FK), content, created_at |

> ※ 리프레시 토큰, 이력 보존용 테이블 및 코드 마스터 테이블을 포함하여 총 **22개** 테이블로 확장됨. 상세 내역은 `docs/erd/ERD_이음.html` 참조.

### 8-2. 엔티티 관계

> 💡 프로젝트가 고도화되면서 전체 관계선 및 테이블 구조가 확장되었습니다 (총 22개 테이블).
> 상세한 컬럼 정보와 테이블 간의 최신 다이어그램 표현은 아래의 웹 문서를 통해 확인할 수 있습니다.

**👉 전체 테이블 다이어그램 확인하기: `docs/erd/ERD_이음.html`**
...
- 관리자 자체 축제 등록/수정 기능 추가 (공공 축제 연동 외에 로컬 행사 자체 관리 수요 반영)
...
- [x] 관리자 자체 축제 등록·관리 추가 (공공 데이터 외에 자체 기획 축제 지원)
...
├── 📢 공지사항 — 비회원/회원 모두 열람
│
├── 📩 문의하기 (회원 전용) — 로그인 사용자만 문의 작성
│   ├── 문의 작성 (제목, 내용)
│   └── 마이페이지에서 내 문의 목록 + 답변 확인
│
├── 👤 마이페이지 (회원 전용)
│   ├── 내 활동 관리
│   │   ├── 즐겨찾기
...
└── ⚙️ 관리자 (ADMIN 전용)
    ├── 축제 데이터 관리
    │   ├── 공공 축제 관리 (API 수동 갱신)
    │   └── 자체 축제 등록·관리 (로컬 행사 운영)
    ├── 운영관리
    │   ├── 사용자 신고관리
    │   ├── 📩 문의 관리 (문의 목록 조회 + 답변 작성)
    │   └── 공지사항 및 팝업 설정
    └── 📊 통계 대시보드 ✅ (필수)
        ├── 지역별 축제 분포 현황
        └── 사용자 유입 및 활동 추이 시각화
